#' Plot Neighbourhood Matches in Side-by-Side Embeddings
#'
#' Visualizes one-to-one neighbourhood matches between two datasets by plotting their
#' reduced dimension embeddings side-by-side with connecting lines showing the matches.
#'
#' @param milos List of 2 Milo objects.
#' @param dt_match A data.table with 3 columns containing the nhood-nhood matching and similarities.
#'   Typically generated by \code{match_nhoods()}.
#' @param cols_color A character vector of length 2 specifying column names from \code{colData}
#'   used to color the neighbourhoods. If \code{NULL}, neighbourhoods are displayed in black.
#' @param dt_palette A data.table with 2 columns (one named after \code{cols_color} containing
#'   neighbourhood labels, one named \code{"color"} containing HEX color codes),
#'   or \code{NULL} to use default colours.
#' @param dimred Character string specifying which reduced dimension to visualize.
#'   Default is \code{"UMAP"}.
#' @param args_process_coordinates A list of 2 lists specifying coordinate preprocessing.
#'   Each nested list contains \code{angle} (rotation angle in degrees) and
#'   \code{shift} (numeric vector of length 2 for horizontal and vertical offset).
#' @param add_non_matched_category Ignored, not implemented yet.
#' @param sim_limits Numeric vector of length 2 specifying colour scale limits for similarities.
#'   If \code{NULL}, limits are determined from the data.
#' @param title Character string for the plot subtitle.
#' @param linewd Numeric. Width of the matching connecting lines. Default is \code{0.35}.
#'
#' @returns A \code{ggplot} object displaying the matched neighbourhoods.
#'
#' @export
#'
#' @examples
#' # Not run: plot_matches_embed(milos, dt_match, cols_color = c("celltype", "celltype"))
