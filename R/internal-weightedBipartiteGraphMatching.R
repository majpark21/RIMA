...
#'
.weightedBipartiteGraphMatching <- function(dt_significant_nhoodnhood, col_sim="sim", col_nhoods1=NULL, col_nhoods2=NULL){
  if(is.null(col_nhoods1)){col_nhoods1 <- colnames(dt_significant_nhoodnhood)[1]}
  if(is.null(col_nhoods2)){col_nhoods2 <- colnames(dt_significant_nhoodnhood)[2]}
  ...
  for(ii in seq_along(original_cols)){
    dt_significant_nhoodnhood[[original_cols[ii]]] <- paste(paste0(".GROUP", ii), dt_significant_nhoodnhood[[original_cols[ii]]], sep="__")
  }

  # vertex type, one atlas is coded as up, the other as down
  dt_types <- list(up=unique(dt_significant_nhoodnhood[[1]]), down=unique(dt_significant_nhoodnhood[[2]]))
  ...
  g <- igraph::graph_from_data_frame(dt_significant_nhoodnhood, directed = F, vertices = dt_types)
