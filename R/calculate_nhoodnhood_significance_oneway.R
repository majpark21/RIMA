...
#' @details
#' This function does:
#' 1. Calculate the true nhood-nhood similarities
#' 2. Scramble neighbourhoods in the 2nd Milo, optionally taking labels to determine the resampling weights.
#' 3. Calculate the similarities between true nhoods from the 1st Milo and the scrambled nhoods from the 2nd Milo
#' 4. Calculate and adjust pvalues for each nhood-nhood edges.
#'
#' @returns
#'
#' @examples
.calculate_nhoodnhood_significance_oneway <- function(milos, name_intact="milo_intact", name_scramble="milo_scramble", dt_sims_true, col_scramble_label="false", n_scrambles, sel_assay="logcounts", sim_method="spearman", adjust, alpha_adjust){
  ...
  l_scrambled_assay_nhoods <- scramble_nhood_avg(
    milos[[2]],  # already subset here for speed up
    replace = TRUE,
    n = n_scrambles,
  ...
  for(ii in 1:n_scrambles){
    ...
    dt_sims_scrambled <- calculate_similarities(
      milos = l_matExpression,
      sim = sim_method
    )
